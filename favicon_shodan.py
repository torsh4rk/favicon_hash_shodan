...
def print_shodan_links(URL, domain, VERBOSE):
    ...
    if hash_favicon != None:
        ...
        print(f'\033[92m-> Search on Shodan (Link 1) => https://www.shodan.io/search?query={query_1} (Dork: http.favicon.hash:{hash_favicon})')
        if VERBOSE == 1:
            os.system(f"shodan search http.favicon.hash:{hash_favicon} " + "--fields ip_str,port --separator \" \" | awk '{print $1\":\"$2}'")
        else:
            pass
        print(f'\033[92m-> Search on Shodan (Link 2) => https://www.shodan.io/search?query={query_2} (Dork: http.favicon.hash:{hash_favicon} + ip:{socket.gethostbyname(domain)})')
        if VERBOSE == 1:
